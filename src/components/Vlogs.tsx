  ...
  return (
    <>
      ...
      <div className="text-center">
        <h2 className={`mt-4 text-black text-[clamp(40px,8vw,64px)] font-[510] leading-none tracking-[-1.28px] transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          Vlogs
        </h2>
        <p className={`text-black text-xl md:text-2xl font-normal tracking-[-0.48px] max-w-3xl mt-4 transition-all duration-700 delay-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
      ...
      <div className="w-full max-w-[1115px] mt-10">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {visibleVideos.map((video, index) => (
            ...
            >
              <div className="bg-white shadow-[0px_4px_20px_rgba(0,0,0,0.25)] flex h-full flex-col rounded-[10px] border border-[rgba(34,34,34,0.08)] p-4 transition-all duration-300 group-hover:scale-[1.015] group-hover:shadow-[0px_6px_30px_rgba(0,0,0,0.2)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                <div className="relative w-full overflow-hidden rounded-lg">
                  <div className="aspect-[4/3] w-full flex items-center justify-center">
                    <img src={video.image} alt={video.title} loading="lazy" className="h-full w-full object-contain" />
                  </div>
                </div>
                <h3 className="text-black text-xl font-semibold leading-6 tracking-tight mt-4">{video.title}</h3>
                <time className="text-[#171717] text-sm font-[510] leading-[1.4] tracking-[-0.45px] mt-2">{video.date}</time>
              </div>
            </article>
          ))}
        </div>
        ...
        >
          <div className="relative w-full max-w-4xl rounded-3xl bg-white p-6 shadow-2xl">
            <button
              ...
              className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full border border-[#e4e7ec] bg-white text-[#475467] hover:bg-[#f2f4f7]"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="mt-4 text-center">
              <h3 className="text-xl font-semibold text-[#101828]">{selectedVideo.title}</h3>
            </div>
          </div>
        </div>
      )}
